...
<body class="bg-slate-900 font-sans antialiased text-slate-800 min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
    ...
    <div class="max-w-md w-full relative z-10">
      ...
      <div class="bg-white border border-slate-800/80 rounded-3xl p-8 shadow-2xl space-y-6">
        ...
        @if(session('error'))
          <div class="p-4 bg-rose-50 border border-rose-200 rounded-2xl text-rose-700 text-xs font-semibold flex items-center gap-3">
            <i class="fa-solid fa-circle-exclamation text-rose-600 text-base shrink-0"></i>
            <span>{{ session('error') }}</span>
          </div>
        @endif

        <form action="{{ url(config('database-studio.path', 'database-studio') . '/login') }}" method="POST" class="space-y-5">
          @csrf
          ...
          <div>
            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Security Password *</label>
            <div class="relative">
              ...
              <input type="password" id="input-password" name="password" required
                ...
                placeholder="••••••••">
              <button type="button" id="btn-toggle-password" class="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition cursor-pointer">
        ...
        <div class="pt-4 border-t border-slate-100 text-center">
          <p class="text-[11px] text-slate-400">
            Default credentials are set in your <code class="bg-slate-100 px-1.5 py-0.5 rounded text-slate-700 font-mono text-[10px]">.env</code>:
          </p>
          <div class="mt-2 p-2.5 bg-slate-50 rounded-xl border border-slate-200/80 font-mono text-[10px] text-slate-600 text-left space-y-1">
